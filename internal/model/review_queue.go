...
func (q ReviewQueueQuery) Validate() error {
	...
	if q.Status != "" && q.Status != SuggestionOpen && q.Status != SuggestionAccepted && q.Status != SuggestionIgnored && q.Status != SuggestionExpired {
		return fmt.Errorf("unknown suggestion status %q", q.Status)
	}
	return nil
}
